...
public class TtlStateFactory<K, N, SV, S extends State, IS extends S> {
	...
	private <OIS, TTLCON, TTLDES> TTLCON createTtlStateContext(TTLDES ttlDescriptor) throws Exception {
		...
		return (TTLCON) new TtlStateContext<>(
			origianlState, ttlConfig, timeProvider, stateDesc.getSerializer(),
			getTtlIncrementalCleanupCallback((InternalKvState<?, ?, ?>) origianlState));
	}
	...
	private Runnable getTtlIncrementalCleanupCallback(InternalKvState<?, ?, ?> origianlState) {
		return incrementalCleanup == null || origianlState.getNamespaceKeyIterator() == null
