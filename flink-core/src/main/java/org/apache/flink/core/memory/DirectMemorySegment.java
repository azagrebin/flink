 ...
 */
@Internal
abstract class DirectMemorySegment extends MemorySegment {
	/**
	...
	DirectMemorySegment(byte[] buffer, Object owner) {
		super(buffer, owner);
	}

	DirectMemorySegment(long offHeapAddress, int size, Object owner) {
		super(offHeapAddress, size, owner);
	}

	public abstract ByteBuffer getOffHeapBuffer();
	...
	public ByteBuffer wrap(int offset, int length) {
		if (address <= addressLimit) {
			if (heapMemory != null) {
				...
				try {
					ByteBuffer wrapper = getOffHeapBuffer().duplicate();
	...
	public void put(int offset, ByteBuffer source, int numBytes) {
		...
		if (source.isDirect()) {
			// copy to the target memory directly
			long sourcePointer = getAddress(source) + sourceOffset;
			long targetPointer = address + offset;
