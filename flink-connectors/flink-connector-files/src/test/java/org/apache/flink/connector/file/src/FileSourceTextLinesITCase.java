...
public class FileSourceTextLinesITCase extends TestLogger {
	...
	public void testBoundedTextFileSource() throws Exception {
		...
		try (MiniCluster miniCluster = createAndStartTestingMiniCluster(new Configuration())) {
	...
	private void testContinuousTextFileSource(boolean failTaskManager) throws Exception {
		...
		try (TestingMiniCluster miniCluster = createAndStartTestingMiniCluster(new Configuration())) {
			...
			for (int i = 1; i < LINES_PER_FILE.length; i++) {
				...
				if (failTaskManager && i == LINES_PER_FILE.length / 3) {
					restartTaskManager(miniCluster);
				}
			}
	...
	private static DataStream<String> createFileSourceStream(
			...
			final File testDir,
			@Nullable final Duration discoveryInterval,
			@Nullable final Duration checkpointingInterval) throws Exception {
		...
		if (discoveryInterval != null) {
			fileSourceBuilder.monitorContinuously(discoveryInterval);
		}
	...
	private static void restartTaskManager(TestingMiniCluster miniCluster) throws Exception {
		try {
	...
	private static TestingMiniCluster createAndStartTestingMiniCluster(
			Configuration configuration) throws Exception {
		TestingMiniCluster miniCluster = new TestingMiniCluster(
