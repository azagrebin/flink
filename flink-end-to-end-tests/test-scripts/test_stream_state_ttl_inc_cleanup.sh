...
TEST=flink-stream-state-ttl-inc-cleanup-test
TEST_PROGRAM_NAME=DataStreamStateTtlIncCleanupTestProgram
...
function job_id() {
    CMD="${FLINK_DIR}/bin/flink run -d -p ${PARALLELISM} ${TEST_PROGRAM_JAR} \
      ...
      --max.allowed.state.size ${MAX_KEY_NUM} \
      --update_generator_source.keyspace ${KEYSPACE}"
    echo "${CMD}"
}
...
wait_oper_metric_num_in_records TtlUpdateOperator.0 ${UPDATE_NUM} 'State TTL incremental cleanup test job'
