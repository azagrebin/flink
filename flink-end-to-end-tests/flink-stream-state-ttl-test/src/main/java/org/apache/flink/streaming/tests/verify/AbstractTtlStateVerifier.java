...
abstract class AbstractTtlStateVerifier<D extends StateDescriptor<S, SV>, S extends State, SV, UV, GV>
	...
	AbstractTtlStateVerifier(D stateDesc) {
	...
	@Override
	public State createState(FunctionInitializationContext context, StateTtlConfiguration ttlConfig) {
	...
	@SuppressWarnings("unchecked")
	@Override
	public GV get(State state) throws Exception {
		return getInternal((S) state);
	}

	abstract GV getInternal(S state) throws Exception;

	@SuppressWarnings("unchecked")
	@Override
	public void update(State state, Object update) throws Exception {
		updateInternal((S) state, (UV) update);
	}

	abstract void updateInternal(S state, UV update) throws Exception;

	@SuppressWarnings("unchecked")
	@Override
	public boolean verify(TtlVerificationContext<?, ?> verificationContextRaw, Time precision) {
	...
	private boolean isWithinPrecision(TtlVerificationContext<UV, GV> verificationContext, Time precision) {
		...
		if (prevUpdates == null || prevUpdates.isEmpty()) {
			return true;
		}
	...
	abstract GV expected(List<TtlValue<UV>> updates, long currentTimestamp);
