...
class TtlAggregatingStateVerifier
	extends AbstractTtlStateVerifier<AggregatingStateDescriptor<Integer, Long, String>, AggregatingState<Integer, String>, Long, Integer, String> {
	...
	@Override
	String getInternal(AggregatingState<Integer, String> state) throws Exception {
		return state.get();
	}

	@Override
	void updateInternal(AggregatingState<Integer, String> state, Integer update) throws Exception {
		state.add(update);
	}

	@Override
	String expected(List<TtlValue<Integer>> updates, long currentTimestamp) {
		if (updates.isEmpty()) {
			return null;
		}
